(function($){$.fn.passwordStrength=function(options){var domName=this,defaults,options,objs,dfunc,regObj;defaults={judgeType:0,color:["gray","orange"],weak:["123456","123123","456456","789789","asdasd","zxczxc","qweqwe"],tips:{"0":"请输入密码","1":"长度不对","2":"弱密码，建议修改","3":"请输入数字+字母组合","success":"可用"},eyes:true,eyesArr:["&#xe638;","&#xe639;"],callback:null};options=$.extend(defaults,options);objs={bar:$("<div>").addClass("m_pBar"),levelBar:$("<p>").addClass("m_pLevelBar"),levelColor:$("<span>").addClass("m_pLevelColor").css("background",options.color[1]),eyes:$("<a>").attr("href","javascript:;").addClass("ic_eyes iconfont"),tips:$("<span>").addClass("u_tips").html(options.tips[0])};dfunc={init:function(){domName.parent().append(objs.bar);objs.bar.append(domName);if(options.eyes==true){objs.eyes.html(options.eyesArr[0]);objs.bar.append(objs.eyes);if($.browser.msie&&$.browser.version<=8.0){objs.eyes.hide();};objs.eyes.mousedown(function(){$(this).html(options.eyesArr[1]);domName.prop("type","text");});objs.eyes.mouseup(function(){$(this).html(options.eyesArr[0]);domName.prop("type","password");});};objs.bar.append(objs.tips);objs.levelBar.append(objs.levelColor);objs.bar.append(objs.levelBar);domName.keyup(function(){dfunc.ver($(this).val());});if(options.callback){options.callback();};},ver:function(num){var vLen=num.length,weakList=options["weak"].length,i=0;if(vLen<6){objs.tips.html(options.tips[1]);dfunc.countResult(0);return false;};for(;i<weakList;i++){if(num==options["weak"][i]){objs.tips.html(options.tips[2]);dfunc.countResult(2);return false;};};if(options.judgeType==1&&(regObj.allNum.test(num)||regObj.allEn.test(num))){objs.tips.html(options.tips[3]);dfunc.countResult(0);return false;};dfunc.countScroe(num);objs.tips.html(options.tips.success);},countScroe:function(num){var score=0,vLen=num.length,rLit=/[a-z]/,rCap=/[A-Z]/,LitBoo=rLit.test(num),rCapBoo=rCap.test(num),rNumLen=num.replace(/\D/g,"").length,rSignLen=num.replace(/[a-zA-Z0-9]/g,"").length,bStr=bAbc=bSign=false;if(vLen<6){score+=0;}else if(vLen>=6&&vLen<=10){score+=10;}else if(vLen>10){score+=25;};if(LitBoo==false&&rCapBoo==false){score+=0;}else if(LitBoo==true&&rCapBoo==true){score+=25;}else{score+=10;};if(rNumLen>0&&rNumLen<=2){score+=10;}else if(rNumLen>2){score+=20;}
...
return LV;},countResult:function(LV){if(LV>=5){objs.levelColor.animate({"width":"100%"},300);}else if(LV==4){objs.levelColor.animate({"width":"66.7%"},300);}else if(LV>1&&LV<4){objs.levelColor.animate({"width":"33.4%"},300);}else{objs.levelColor.animate({"width":0},300);}}};regObj={allNum:/^[1-9]\d*$/,allEn:/^[A-Za-z]+$/};dfunc.init(options,objs);};})(jQuery);